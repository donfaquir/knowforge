...
async fn open_workspace(
    ...
    *guard = Some(canonical_root.clone());
    ...
    // Latent paragraph scan: run when embedding index exists but candidates
    // are empty or were generated by an outdated filter version.
    {
        ...
        std::thread::spawn(move || {
            ...
            // Check filter version — clears outdated candidates if needed
            let version_changed = latent_paragraphs::invalidate_if_filter_changed(&conn).unwrap_or(false);
            ...
            if !has_candidates {
                use tauri::Manager;
                eprintln!(
                    "[open_workspace] triggering latent scan (version_changed={version_changed}, no active candidates)"
                );
